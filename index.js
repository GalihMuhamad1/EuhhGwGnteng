/*
	...
	あ JASA RUN HEROKU BISA PC wa.me/6281222180225 BIAR BOT NYA GAK TERLALU DELAY
...
img = 'thumbnail_1.jpg'
baper = 'IG : galihmalgiah'
apiku = 'https://www.instagram.com/galihmalgiah/'
  ...
  //FUNCTION
    ...
    if (isGroup && isRegister && !isCmd) {
						...
						try {
							...
							const amountXp = Math.floor(Math.random() * 9999) + 9999
...
NOTE : 
- Harga Limit Per9999 adalah $1 balance`)
break
case 'buylimit':{
if (!q) return reply(`Kirim perintah *${prefix}buylimit* jumlah limit yang ingin dibeli\n\nHarga 999999999 limit = $1balance`)
...
break
case 'buyglimit':{
if (!q)return reply(`Example : ${prefix + command} 10\n\nHarga 999999999 limit = $100 balance`)
if (q.includes('-')) return reply(`Jangan menggunakan -`)
if (isNaN(q)) return reply(`Harus berupa angka`)
const koinPerlimit = 999999999
...
break
 ...
 let ran_blc = randomNomor(99999999)
...
`
if(menusimple == false){
sendButDoc(from, ini_anu, `𝐏𝐥𝐞𝐚𝐬𝐞 𝐝𝐨??'𝐭 ??𝐩𝐚𝐦 𝐛𝐨𝐭, 𝐩𝐚𝐮𝐬?? 𝟓 𝐬𝐞𝐜𝐨𝐧𝐝𝐬 𝐩𝐞𝐫 𝐜𝐨𝐦𝐦𝐚𝐧𝐝.\n𝐓𝐡𝐚𝐧𝐤𝐬!`, sender, koko, ini_mark)
...
case 'setexif':
		    ...
		    case 'colong':
		            ...
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `GALIHMALGIAH`
...
break
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih kak, jgn lupa Support IG : Galihmalgiah```')
					fs.unlinkSync(ran)
					})
					break
